...
class Latent_AE_cnn_big(nn.Module):
    ...
    def forward(self, input):
        ...
        # import pdb; pdb.set_trace()
        # import pdb; pdb.set_trace()
        input = self.add_noise(input, self.input_noise_factor)
        emb_enc1 = self.enc1(input)
        emb_enc2 = self.enc2(emb_enc1)
        emb_enc3 = self.enc3(emb_enc2)
        emb_enc4 = self.enc4(emb_enc3)

        #emb_enc4 = emb_enc4 + torch.randn_like(emb_enc4) * self.latent_noise_factor
        emb_enc4 = self.add_noise(emb_enc4, self.latent_noise_factor)
    ...
    def decode(self, emb_enc4):
        ...
        emb_dec2 = self.dec2(emb_dec1)
        emb_dec3 = self.dec3(emb_dec2)
        emb_dec4 = self.dec4(emb_dec3)[:, :, :self.in_dim].squeeze(1)
